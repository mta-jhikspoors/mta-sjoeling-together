...
MultiplayerHUDRenderer::MultiplayerHUDRenderer(ParticleOverlayRenderer& particlesoverlay) :
	player1score(Main::GetResources().GetFont("pixel7.fnt"), HorizontalAlign::Left, VerticalAlign::Middle),
	player2score(Main::GetResources().GetFont("pixel7.fnt"), HorizontalAlign::Right, VerticalAlign::Middle),
	pucks(Main::GetResources().GetFont("pixel7.fnt"), HorizontalAlign::Center, VerticalAlign::Middle),
	player1label(Main::GetResources().Smallest(), HorizontalAlign::Left, VerticalAlign::Top),
	player2label(Main::GetResources().Smallest(), HorizontalAlign::Right, VerticalAlign::Top),
...
gaterequired{ {false, false, false, false} }
{
	...
	player1score.SetText(String::From(0));
	player2score.SetText(String::From(0));
...
void MultiplayerHUDRenderer::Render(Canvas& canvas)
{
	...
	if (!ch::IsTimeSet(fadeoutstart) || (t < (fadeoutstart + FADE_OUT_DURATION)))
	{
		...
		// Draw counters
		player1score.DrawOutlineBlend(canvas, Point(ROUND_X, COUNTERS_Y), 1, Color(COUNTERS_OUTLINE, alpha));
		player2score.DrawOutlineBlend(canvas, Point(SCORE_X, COUNTERS_Y), 1, Color(COUNTERS_OUTLINE, alpha));
		pucks.DrawOutlineBlend(canvas, Point(PUCKS_X, COUNTERS_Y), 1, Color(COUNTERS_OUTLINE, alpha));
		player1score.DrawTexturedModBlend(canvas, Point(ROUND_X, COUNTERS_Y), texture, Color(255, 255, 255, alpha));
		player2score.DrawTexturedModBlend(canvas, Point(SCORE_X, COUNTERS_Y), texture, Color(255, 255, 255, alpha));
		pucks.DrawTexturedModBlend(canvas, Point(PUCKS_X, COUNTERS_Y), texture, Color(255, 255, 255, alpha));

		// Draw counter labels
		player1label.DrawOutlineBlend(canvas, Point(ROUND_X, LABELS_Y), 1, Color(LABELS_OUTLINE, alpha));
		player2label.DrawOutlineBlend(canvas, Point(SCORE_X, LABELS_Y), 1, Color(LABELS_OUTLINE, alpha));
		puckslabel.DrawOutlineBlend(canvas, Point(PUCKS_X, LABELS_Y), 1, Color(LABELS_OUTLINE, alpha));
		player1label.DrawBlend(canvas, Point(ROUND_X, LABELS_Y), Color(LABELS_COLOR, alpha));
		player2label.DrawBlend(canvas, Point(SCORE_X, LABELS_Y), Color(LABELS_COLOR, alpha));
...
void MultiplayerHUDRenderer::SetRequiredGates(bool* gates)
{
	for (int g = 0; g < GAME_GATES; g++)
		gaterequired[g] = gates[g];
}

void MultiplayerHUDRenderer::ScoreRequiredGate(int gate)
{
	switch (gate)
	{
	case 0:
...
void MultiplayerHUDRenderer::ScoreGate(int gate)
{
	switch (gate)
	{
	case 0:
...
void MultiplayerHUDRenderer::ScoreSet()
{
	if (!ch::IsTimeSet(scoresettime))
		scoresettime = Clock::now() + ch::milliseconds(SCORE_SET_DELAY);
}

void MultiplayerHUDRenderer::DrawGateLine(Canvas& canvas, int startx, int endx, Color color)
{
	// Outline on the left
...
void MultiplayerHUDRenderer::SpawnGateEffectBig(int startx, int endx)
{
...
void MultiplayerHUDRenderer::SpawnGateEffectSmall(int startx, int endx)
{
	int center = (startx + endx) / 2;
...
void MultiplayerHUDRenderer::SpawnGateParticle(int x, int y, int centerx, Color color)
{
...
void MultiplayerHUDRenderer::FloatPoints(String text, int x, int y)
{
