...
ClassicPlayingState::ClassicPlayingState(GameStateMachine* _statemachine) :
...
void ClassicPlayingState::Enter()
{
...
void ClassicPlayingState::Leave()
{
}

void ClassicPlayingState::Update()
{
...
bool ClassicPlayingState::HandleMessage(const IOModule_IOMessage& msg)
{
	GameData& gd = statemachine->GetData();
...
void ClassicPlayingState::UpdateDisplay()
{
	GameData& gd = statemachine->GetData();
	RoundData& rd = gd.CurrentRound();
	bool gatesrequired[GAME_GATES];
...
void ClassicPlayingState::PlayShotSound()
{
...
void ClassicPlayingState::KillShotSounds()
{
	for(int i = 1; i <= NUM_SHOT_SOUNDS; i++)
		Main::GetResources().GetSound("shot" + String::From(i) + ".wav").Stop();
}

void ClassicPlayingState::CheckComboAchievement()
{
